...
function runMigrations(db: Database.Database): void {
  ...
  const migrations: Array<() => void> = [
    ...
    // [460-fork] Partner pairing (Milestone 3): symmetric spouse/partner link on users table.
    // Symmetry is enforced by application logic inside a DB transaction (see partnerService.ts).
    // Keep this thunk at the TAIL of the array on upstream rebase.
    () => {
      try {
    ...
    // [460-fork] Partner pairing (Milestone 3): time-limited invite tokens between users.
